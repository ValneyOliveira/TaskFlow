...
const AppSidebar = () => {
  ...
  return (
    <Sidebar.Sidebar collapsible='icon'>
      <Sidebar.SidebarContent className='items-center outline outline-neutral-400'>
            ...
            <Sidebar.SidebarMenuItem key={item.title}>
              <Sidebar.SidebarMenuButton asChild>
                <Link href={item.url} className='hover:bg-blue-50'>
                  <item.icon className={`${path === item.url? 'text-blue-600' : 'text-neutral-500'}`}/>
                  <span className={`${path === item.url? 'text-blue-600' : 'text-neutral-500'}`}>{item.title}</span>
